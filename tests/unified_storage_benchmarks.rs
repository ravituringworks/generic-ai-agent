...
impl StorageBenchmark {
    ...
    pub async fn benchmark_workflow_operations(&self, num_operations: usize) -> Vec<BenchmarkResults> {
        ...
        for i in 0..num_operations {
            ...
            self.storage.store_suspended_workflow(&workflow).await.unwrap();
            latencies.push(start.elapsed());
        }
        results.push(BenchmarkResults::new("Workflow Storage".to_string(), num_operations, latencies));
        ...
        for i in 0..num_operations {
            ...
            let start = Instant::now();
            let _retrieved = self.storage.get_suspended_workflow(&workflow_id).await.unwrap();
            latencies.push(start.elapsed());
        }
        results.push(BenchmarkResults::new("Workflow Retrieval".to_string(), num_operations, latencies));
        
        // Benchmark workflow listing
        let start = Instant::now();
        let _workflows = self.storage.list_suspended_workflows(&resource_id).await.unwrap();
        let list_duration = start.elapsed();
        results.push(BenchmarkResults::new("Workflow Listing".to_string(), 1, vec![list_duration]));
        
        results
    }
    
    // Benchmark memory operations
    pub async fn benchmark_memory_operations(&self, num_threads: usize, messages_per_thread: usize) -> Vec<BenchmarkResults> {
        let mut results = Vec::new();
        ...
        results.push(BenchmarkResults::new("Thread Creation".to_string(), num_threads, latencies));
        ...
        for (thread_idx, thread_id) in thread_ids.iter().enumerate() {
            for msg_idx in 0..messages_per_thread {
                let message = MemoryMessage {
                    ...
                    resource_id: resource_id.clone(),
                    role: if msg_idx % 2 == 0 { MessageRole::User } else { MessageRole::Assistant },
        ...
        results.push(BenchmarkResults::new("Message Storage".to_string(), num_threads * messages_per_thread, latencies));
        
        // Benchmark message retrieval
        let mut latencies = Vec::new();
        for thread_id in &thread_ids {
            let start = Instant::now();
            let _messages = self.storage.get_memory_messages(thread_id, None).await.unwrap();
            latencies.push(start.elapsed());
        }
        results.push(BenchmarkResults::new("Message Retrieval".to_string(), num_threads, latencies));
    ...
    pub async fn benchmark_trace_operations(&self, num_traces: usize) -> Vec<BenchmarkResults> {
        ...
        for i in 0..num_traces {
            ...
            let trace = TraceData {
                ...
                status: if i % 10 == 0 { 
                    TraceStatus::Error { message: format!("Test error {}", i) } 
                } else { 
                    TraceStatus::Ok 
                },
                attributes: {
        ...
        results.push(BenchmarkResults::new("Trace Storage".to_string(), num_traces, latencies));
        
        // Benchmark trace querying (all traces)
        let start = Instant::now();
        let _traces = self.storage.query_traces(&resource_id, TraceFilters::default()).await.unwrap();
        let query_duration = start.elapsed();
        results.push(BenchmarkResults::new("Trace Query (All)".to_string(), 1, vec![query_duration]));
        ...
        for component in &components {
            ...
            let start = Instant::now();
            let _traces = self.storage.query_traces(&resource_id, filters).await.unwrap();
            latencies.push(start.elapsed());
        }
        results.push(BenchmarkResults::new("Trace Query (Filtered)".to_string(), components.len(), latencies));
        
        results
    }
    
    // Benchmark evaluation operations
    pub async fn benchmark_evaluation_operations(&self, num_datasets: usize, num_scores: usize) -> Vec<BenchmarkResults> {
        let mut results = Vec::new();
        ...
        results.push(BenchmarkResults::new("Dataset Creation".to_string(), num_datasets, latencies));
        
        // Benchmark score storage
        let metrics = vec!["accuracy", "precision", "recall", "f1_score", "bleu", "rouge"];
        let mut latencies = Vec::new();
        ...
        results.push(BenchmarkResults::new("Score Storage".to_string(), num_scores, latencies));
        ...
        results.push(BenchmarkResults::new("Score Retrieval".to_string(), num_runs, latencies));
        
        results
    }
    
    // Benchmark concurrent operations
    pub async fn benchmark_concurrent_operations(&self, num_concurrent: usize, operations_per_task: usize) -> BenchmarkResults {
    ...
    pub async fn benchmark_cleanup_operations(&self) -> BenchmarkResults {
        ...
        let start = Instant::now();
        let _cleanup_stats = self.storage.cleanup_old_data(&retention_policy).await.unwrap();
...
mod benchmark_tests {
    ...
    async fn test_workflow_operations_benchmark() {
        ...
        for result in &results {
            ...
            assert!(result.ops_per_second > 1000.0, "Workflow operations should be > 1000 ops/sec");
            assert!(result.avg_latency_ms < 10.0, "Average latency should be < 10ms");
        }
    }
    ...
    async fn test_memory_operations_benchmark() {
        ...
        for result in &results {
            ...
            assert!(result.avg_latency_ms < 5.0, "Memory operations should be < 5ms average latency");
        }
    }
    ...
    async fn test_trace_operations_benchmark() {
        ...
        for result in &results {
            ...
            if result.operation_name.contains("Storage") {
                assert!(result.ops_per_second > 5000.0, "Trace storage should be > 5000 ops/sec");
            }
        }
    }
    ...
    async fn test_evaluation_operations_benchmark() {
        ...
        for result in &results {
            ...
            assert!(result.avg_latency_ms < 5.0, "Evaluation operations should be < 5ms average");
        }
    }
    ...
    async fn test_concurrent_operations_benchmark() {
        ...
        assert!(result.ops_per_second > 1000.0, "Concurrent operations should be > 1000 ops/sec");
    }
    ...
    async fn test_storage_statistics_benchmark() {
        ...
        assert!(result.avg_latency_ms < 1.0, "Statistics should be < 1ms average");
    }
    ...
    async fn test_load_test_scenario() {
        ...
        assert!(result.ops_per_second > 500.0, "Load test should achieve > 500 ops/sec");
        assert!(result.total_duration.as_secs() < 30, "Load test should complete in < 30 seconds");
        
        // Verify data integrity after load test
        let stats = benchmark.storage.get_storage_stats().await.unwrap();
        assert!(stats.suspended_workflows >= 1000, "Should have stored workflow operations");
        assert!(stats.traces >= 1500, "Should have stored trace operations");
        
        println!("✅ Load test passed - system handled {} operations in {:?}", 
...
mod integration_benchmark_tests {
    ...
    async fn test_realistic_application_scenario() {
        ...
        benchmark.storage_manager.suspend_workflow(
        ...
        let thread_id = benchmark.storage_manager.create_conversation_thread(
            resource_id.clone(),
        ...
        let conversation_pairs = vec![
            ...
            ("Great! Let's begin with setting up your profile.", "assistant"),
        ...
        for (content, role) in conversation_pairs {
            ...
            benchmark.storage_manager.add_message(
                &thread_id,
                resource_id.clone(),
                message_role,
                content,
        ...
        for (component, operation, duration_ms) in operations {
            ...
            benchmark.storage_manager.record_trace(
                resource_id.clone(),
                component,
                operation,
                now,
        ...
        let dataset_id = benchmark.storage_manager.create_evaluation_dataset(
            resource_id.clone(),
        ...
        for (metric, score) in eval_metrics {
            benchmark.storage_manager.record_evaluation_score(
        ...
        let (context, step) = benchmark.storage_manager.resume_workflow("user_onboarding_flow").await.unwrap();
        assert_eq!(step, 0);
        ...
        println!("   - Conversation: {} messages stored", conversation_pairs.len());
        ...
        assert!(total_duration.as_secs() < 5, "Realistic scenario should complete quickly");
        assert!(stats.memory_threads >= 1, "Should have conversation threads");
        assert!(stats.traces >= operations.len(), "Should have recorded traces");
        assert!(stats.eval_scores >= eval_metrics.len(), "Should have evaluation scores");
    }
}
