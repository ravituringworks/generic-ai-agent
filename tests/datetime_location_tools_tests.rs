...
use generic_ai_agent::tools::{execute_datetime_info, execute_location_info, BuiltinTools};
...
async fn test_datetime_info_tool() {
    ...
    if let Some(content) = result.content.first() {
        match content {
            generic_ai_agent::mcp::ToolContent::Text { text } => {
            ...
            generic_ai_agent::mcp::ToolContent::Text { text } => {
                ...
                if let Some(json_start) = text.find("{") {
                    ...
                    if let Some(json_end) = json_str.rfind("}") {
                        ...
                        assert!(parsed.is_ok(), "Failed to parse datetime JSON: {}", json_str);
                        ...
                        assert!(parsed_datetime.is_ok(), "Failed to parse ISO8601 UTC datetime: {}", iso8601_utc);
                    }
                }
            }
            _ => panic!("Expected text content"),
            ...
            generic_ai_agent::mcp::ToolContent::Text { text } => {
                ...
                if let Some(json_start) = text.find("{") {
                    ...
                    if let Some(json_end) = json_str.rfind("}") {
                        ...
                        // Parse the JSON to ensure it's valid
                        let parsed: Result<serde_json::Value, _> = serde_json::from_str(json_str);
                        assert!(parsed.is_ok(), "Failed to parse location JSON: {}", json_str);
...
mod integration_tests {
    ...
    async fn test_datetime_and_location_tools_are_async() {
        ...
        let (datetime_result, location_result) = tokio::join!(
