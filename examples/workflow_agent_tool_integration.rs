...
impl Tool for DataTransformTool {
    async fn execute(&self, input: &ToolInput) -> Result<ToolResult> {
        println!(
            "  🔧 Tool '{}' executing {} transformation",
            self.name, self.transformation_type
        );
    ...
    fn description(&self) -> &str {
        &format!(
            "Performs {} transformation on text data",
...
impl WorkflowStep for AgentInvokingStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        context
            .metadata
            .insert(format!("{}_response", self.name), response);
...
impl WorkflowStep for ToolInvokingStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        context
            .metadata
            .insert(format!("{}_result", self.name), result.data().to_string());
...
pub fn create_text_processing_workflow(llm_client: Arc<dyn LLMClient>) -> WorkflowBuilder {
    ...
    let sentiment_agent = Arc::new(TextAnalysisAgent::new(
        "sentiment_agent",
        llm_client.clone(),
    ));
...
async fn main() -> anyhow::Result<()> {
    ...
    let summary_agent = Arc::new(TextAnalysisAgent::new(
        "text_summarizer",
        llm_client.clone(),
    ));
    ...
    let agent_workflow = WorkflowBuilder::new("agent_integration_demo")
        .with_initial_data(
            json!({"current_text": "This is an amazing product that exceeds expectations!"}),
        )
    ...
    println!(
        "Agent workflow completed: Steps executed = {}\n",
    ...
    println!(
        "Tool workflow completed: Steps executed = {}\n",
        result.steps_executed
    );
    ...
    let prompt_mapper: MapperFn = Arc::new(|context, input_data| {
        ...
        context
            .metadata
            .insert("formatted_prompt".to_string(), prompt.clone());
    ...
    let mapped_workflow = WorkflowBuilder::new("mapped_input_demo")
        ...
        .map(prompt_mapper)
        .then(create_agent_step(
            "sentiment_with_mapping",
            Arc::new(TextAnalysisAgent::new(
                "sentiment_mapper",
                llm_client.clone(),
            )),
        ))
    ...
    let analysis_agent = Arc::new(TextAnalysisAgent::new(
