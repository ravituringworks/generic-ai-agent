...
impl TextAnalysisStep {
    ...
    async fn generate_analysis(&self, input: &str) -> Result<String> {
        ...
        let analysis = match self.analysis_type.as_str() {
            "sentiment" => {
                if input.contains("amazing") || input.contains("great") || input.contains("excellent") {
                    "Sentiment: POSITIVE (confidence: 0.95)"
                } else if input.contains("bad") || input.contains("terrible") || input.contains("awful") {
...
impl WorkflowStep for TextAnalysisStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        println!("  🤖 Agent-like step '{}' performing {} analysis", self.name, self.analysis_type);
        ...
        context.metadata.insert("current_text".to_string(), analysis_result.clone());
        
        println!("  ✅ Agent-like step '{}' completed: {}", self.name, analysis_result);
...
impl WorkflowStep for DataTransformStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        println!("  🔧 Tool-like step '{}' executing {} transformation", self.name, self.transform_type);
        ...
        if let Some(result_value) = tool_result.get("result") {
            ...
            context.metadata.insert("current_text".to_string(), result_text);
        }
        
        println!("  ✅ Tool-like step '{}' completed transformation", self.name);
        Ok(WorkflowDecision::Continue)
    }
...
pub fn create_analysis_step(name: &str, analysis_type: &str) -> Box<dyn WorkflowStep + Send + Sync> {
    Box::new(TextAnalysisStep::new(name, analysis_type))
}

/// Helper function to create tool-like steps (analogous to createStep(tool))
pub fn create_transform_step(name: &str, transform_type: &str) -> Box<dyn WorkflowStep + Send + Sync> {
...
pub fn create_text_processing_workflow() -> WorkflowBuilder {
    WorkflowBuilder::new("text_processing_workflow")
        ...
        .with_initial_data(json!({"current_text": "This is an amazing product with excellent features!"}))
}
...
impl WorkflowStep for HybridProcessingStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        println!("  🔄 Hybrid step '{}' performing combined processing", self.name);
        ...
        context.metadata.insert("current_text".to_string(), combined_result);
        
        println!("  ✅ Hybrid step '{}' completed combined processing", self.name);
        Ok(WorkflowDecision::Continue)
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    let agent_workflow = WorkflowBuilder::new("agent_like_demo")
        .with_initial_data(json!({"current_text": "This amazing product has excellent features that work great!"}))
    ...
    println!("Agent-like workflow completed: Steps executed = {}\n", result.steps_executed);
    ...
    println!("Tool-like workflow completed: Steps executed = {}\n", result.steps_executed);
    ...
    println!("Hybrid workflow completed: Steps executed = {}\n", result.steps_executed);
