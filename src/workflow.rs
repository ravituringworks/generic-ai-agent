...
impl StepSchema {
    ...
    pub fn add_property(mut self, name: &str, property_type: &str) -> Self {
        self.properties.insert(name.to_string(), serde_json::json!({
            "type": property_type
        }));
        self
    }
...
pub type ConditionFn = Arc<dyn Fn(&WorkflowContext, Option<&serde_json::Value>) -> bool + Send + Sync>;

/// Data mapping function
pub type MapperFn = Arc<dyn Fn(&WorkflowContext, &serde_json::Value) -> serde_json::Value + Send + Sync>;
...
impl WorkflowBuilder {
    ...
    pub fn parallel(self, steps: Vec<Box<dyn WorkflowStep + Send + Sync>>) -> ParallelWorkflowBuilder {
...
impl EventBus {
    ...
    pub fn send_event(&self, event: WorkflowEvent) -> Result<usize> {
        ...
        let sender = broadcasters.entry(event.event_type.clone())
        ...
        if subscriber_count > 0 {
            match sender.send(event.clone()) {
                Ok(_) => {
                    info!("Sent event '{}' to {} subscribers", event.event_type, subscriber_count);
                    Ok(subscriber_count)
                }
                Err(_) => {
                    warn!("Failed to send event '{}' - no active receivers", event.event_type);
                    Ok(0)
                }
            }
        } else {
    ...
    pub fn subscribe(&self, event_type: &str) -> broadcast::Receiver<WorkflowEvent> {
        ...
        let sender = broadcasters.entry(event_type.to_string())
            .or_insert_with(|| broadcast::channel(self.channel_capacity).0);
    ...
    pub async fn wait_for_event(&self, event_id: &str, timeout_ms: Option<u64>) -> Result<Option<WorkflowEvent>> {
...
impl WorkflowContext {
    ...
    pub fn mark_memories_retrieved(&mut self) {
        self.metadata.insert("memories_retrieved".to_string(), "true".to_string());
    }
}
...
impl SnapshotStorage for FileSnapshotStorage {
    async fn store_snapshot(&self, snapshot: &WorkflowSnapshot) -> Result<()> {
        ...
        if !self.storage_dir.exists() {
            fs::create_dir_all(&self.storage_dir).await
                .map_err(|e| AgentError::Workflow(format!("Failed to create snapshot directory: {}", e)))?;
        }
    ...
    async fn list_snapshots(&self, filter: Option<HashMap<String, String>>) -> Result<Vec<WorkflowSnapshot>> {
        ...
        let mut dir = fs::read_dir(&self.storage_dir).await
            .map_err(|e| AgentError::Workflow(format!("Failed to read snapshot directory: {}", e)))?;
    ...
    async fn delete_snapshot(&self, id: Uuid) -> Result<bool> {
        ...
        if path.exists() {
            fs::remove_file(&path).await
                .map_err(|e| AgentError::Workflow(format!("Failed to delete snapshot file: {}", e)))?;
...
pub trait SuspendableWorkflowStep: WorkflowStep {
    ...
    async fn restore_state(&self, _context: &mut WorkflowContext, _state: Option<&serde_json::Value>) -> Result<()> {
...
impl WorkflowStep for EnhancedMemoryRetrievalStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        if context.metadata.contains_key("external_memory_unavailable") {
            return Ok(WorkflowDecision::Suspend(SuspendReason::WaitingForResource("External memory service unavailable".to_string())));
        }
    ...
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        if let Some(approval) = context.metadata.get("human_approval") {
            if approval == "granted" {
                ...
                return Ok(WorkflowDecision::Complete("Workflow stopped by user denial.".to_string()));
            }
        }
        
        // No approval yet, suspend and wait
        Ok(WorkflowDecision::Suspend(SuspendReason::WaitingForInput(self.approval_message.clone())))
    }
...
impl WorkflowStep for RateLimitedApiStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        if let Some(last_call) = context.metadata.get(&format!("{}_last_call", self.api_name)) {
...
impl WorkflowStep for WaitForEventStep {
    async fn execute(&self, _context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        debug!("Wait for event step executing for event '{}'", self.event_id);
...
impl WorkflowStep for ConditionalPauseStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        debug!("Conditional pause step checking condition '{}'", self.condition_key);
        ...
        if let Some(condition_value) = context.metadata.get(&self.condition_key) {
            if condition_value == "true" {
                match &self.pause_type {
                    ...
                    PauseType::WaitForEvent { event_id, timeout_ms } => {
    ...
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        for (i, step) in self.steps.iter().enumerate() {
            ...
            match step.execute(&mut context_clone).await {
                Ok(decision) => {
                    info!("Parallel step {} completed with decision: {:?}", i, decision);
            ...
            LoopType::DoWhile => {
                ...
                loop {
                    ...
                    if iteration > MAX_ITERATIONS {
                        warn!("Loop exceeded maximum iterations ({}), breaking", MAX_ITERATIONS);
                        break;
                    }
...
impl WorkflowStep for ForEachExecutionStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        for (index, item) in items.iter().enumerate() {
            ...
            context.metadata.insert("foreach_current_item".to_string(), item.to_string());
            context.metadata.insert("foreach_current_index".to_string(), index.to_string());
...
impl WorkflowStep for MapExecutionStep {
    async fn execute(&self, context: &mut WorkflowContext) -> Result<WorkflowDecision> {
        ...
        let input_data = context.metadata.get("last_result")
...
impl WorkflowEngine {
    ...
    pub async fn resume_from_snapshot(&self, snapshot_id: Uuid) -> Result<WorkflowResult> {
        ...
        let snapshot = storage.get_snapshot(snapshot_id).await?
            .ok_or_else(|| AgentError::Workflow(format!("Snapshot not found: {}", snapshot_id)))?;
        
        info!("Resuming workflow from snapshot: {} (created: {})", snapshot_id, snapshot.created_at);
    ...
    async fn execute_from_step(&self, mut context: WorkflowContext, start_step: usize) -> Result<WorkflowResult> {
            ...
            for (step_index, step) in self.steps.iter().enumerate().skip(start_step) {
                ...
                if self.suspend_config.auto_checkpoint && 
                   step_index % self.suspend_config.checkpoint_interval == 0 {
                    if let Err(e) = self.suspend(&context, step_index, SuspendReason::Scheduled).await {
                ...
                match step.execute(&mut context).await? {
                    ...
                    WorkflowDecision::WaitForInput(message) => {
                        ...
                        let step_count = context.step_count;
                        return Ok(WorkflowResult {
                            response: format!("Waiting for input: {} (Suspended with ID: {})", message, snapshot_id),
                            context,
                            completed: false,
                            steps_executed: step_count,
                            pending_tool_calls: None,
                    ...
                    WorkflowDecision::Sleep(duration_ms) => {
                        ...
                        let reason = SuspendReason::Sleep { duration_ms, started_at };
                        let snapshot_id = self.suspend(&context, step_index, reason).await?;
                        
                        // Note: Auto-resume would require shared ownership of engine
                        // For now, we'll just suspend and let external code handle resume
                        info!("Workflow suspended for sleep, use resume_from_snapshot({}) after {}ms", snapshot_id, duration_ms);
                        
                        let step_count = context.step_count;
                        return Ok(WorkflowResult {
                            response: format!("Sleeping for {}ms (Suspended with ID: {})", duration_ms, snapshot_id),
                            context,
                            completed: false,
                            steps_executed: step_count,
                            pending_tool_calls: None,
                    ...
                    WorkflowDecision::SleepUntil(timestamp) => {
                        ...
                        let step_count = context.step_count;
                        return Ok(WorkflowResult {
                            response: format!("Sleeping until {} (Suspended with ID: {})", timestamp, snapshot_id),
                            context,
                            completed: false,
                            steps_executed: step_count,
                            pending_tool_calls: None,
                    ...
                    WorkflowDecision::WaitForEvent { event_id, timeout_ms } => {
                        info!("Workflow waiting for event '{}' with timeout {:?}ms", event_id, timeout_ms);
                        let started_at = Utc::now();
                        ...
                        let step_count = context.step_count;
                        return Ok(WorkflowResult {
                            response: format!("Waiting for event '{}' (Suspended with ID: {})", event_id, snapshot_id),
                            context,
                            completed: false,
                            steps_executed: step_count,
                            pending_tool_calls: None,
    ...
    async fn cleanup_snapshots(&self) -> Result<()> {
        if let Some(ref storage) = self.snapshot_storage {
            ...
            if snapshots.len() > self.suspend_config.max_snapshots {
                ...
                for snapshot in snapshots.iter().skip(self.suspend_config.max_snapshots).take(to_delete) {
                    storage.delete_snapshot(snapshot.id).await?;
                }
            }
            
            // Remove old snapshots based on retention policy
            storage.cleanup_old_snapshots(self.suspend_config.snapshot_retention).await?;
        }
    ...
    pub async fn list_snapshots(&self, filter: Option<HashMap<String, String>>) -> Result<Vec<WorkflowSnapshot>> {
        if let Some(ref storage) = self.snapshot_storage {
    ...
    pub async fn store_snapshot(&self, snapshot: &WorkflowSnapshot) -> Result<()> {
        if let Some(ref storage) = self.snapshot_storage {
            ...
            Err(AgentError::Workflow("No snapshot storage configured".to_string()))
        }
    }
    ...
    pub async fn wait_for_event(&self, event_id: &str, timeout_ms: Option<u64>) -> Result<Option<WorkflowEvent>> {
        info!("Waiting for event '{}' with timeout {:?}ms", event_id, timeout_ms);
    ...
    pub fn send_simple_event(&self, event_id: &str, event_type: &str, payload: serde_json::Value) -> Result<usize> {
...
mod tests {
    ...
    async fn test_workflow_context() {
        ...
        assert_eq!(context.metadata.get("memories_retrieved"), Some(&"true".to_string()));
    }
    ...
    async fn test_human_approval_step() {
        ...
        let decision = step.execute(&mut context).await.unwrap();
        assert!(matches!(decision, WorkflowDecision::Suspend(SuspendReason::WaitingForInput(_))));
        
        // Approval granted
        context.metadata.insert("human_approval".to_string(), "granted".to_string());
        let decision = step.execute(&mut context).await.unwrap();
        assert!(matches!(decision, WorkflowDecision::Continue));
        
        // Approval denied
        context.metadata.insert("human_approval".to_string(), "denied".to_string());
        let decision = step.execute(&mut context).await.unwrap();
    ...
    async fn test_rate_limited_api_step() {
        ...
        let decision = step.execute(&mut context).await.unwrap();
        assert!(matches!(decision, WorkflowDecision::Suspend(SuspendReason::RateLimit)));
    }
    ...
    async fn test_enhanced_memory_retrieval_step() {
        ...
        let decision = step.execute(&mut context).await.unwrap();
        assert!(matches!(decision, WorkflowDecision::Suspend(SuspendReason::WaitingForResource(_))));
    }
    ...
    async fn test_workflow_engine_with_suspend_resume() {
        ...
        let engine = WorkflowEngine::new()
            ...
            .add_step(Box::new(HumanApprovalStep::new("Test approval".to_string())));
        ...
        assert!(matches!(snapshot.suspend_reason, SuspendReason::WaitingForInput(_)));
        ...
        let new_snapshot = engine.create_snapshot(&resumed_context, snapshot.current_step, SuspendReason::Manual).await.unwrap();
    ...
    async fn test_snapshot_cleanup() {
        ...
        // Test cleanup
        let cleaned = storage.cleanup_old_snapshots(chrono::Duration::days(1)).await.unwrap();
    ...
    async fn test_wait_for_event_step() {
        ...
        if let WorkflowDecision::WaitForEvent { event_id, timeout_ms } = decision {
    ...
    async fn test_conditional_pause_step() {
        let step = ConditionalPauseStep::new(
        ...
        // Test condition met
        context.metadata.insert("should_pause".to_string(), "true".to_string());
        let decision = step.execute(&mut context).await.unwrap();
    ...
    async fn test_workflow_engine_pause_methods() {
        ...
        // Test event sending
        let sent_count = engine.send_simple_event(
            "test",
            "test_type",
    ...
    async fn test_workflow_with_sleep_step() {
        ...
        let snapshot = &snapshots[0];
        assert!(matches!(snapshot.suspend_reason, SuspendReason::Sleep { .. }));
    }
    ...
    async fn test_workflow_with_event_step() {
        ...
        let engine = WorkflowEngine::new()
            ...
            .add_step(Box::new(WaitForEventStep::new("user_action".to_string(), Some(1000))));
        ...
        let snapshot = &snapshots[0];
        assert!(matches!(snapshot.suspend_reason, SuspendReason::WaitingForEvent { .. }));
    }
    ...
    async fn test_parallel_execution_step() {
        let steps: Vec<Box<dyn WorkflowStep + Send + Sync>> = vec![
            Box::new(SleepStep::new(10)),
            Box::new(SleepStep::new(20)),
        ];
    ...
    async fn test_branch_execution_step() {
        ...
        let condition: ConditionFn = Arc::new(|context, _| {
            context.metadata.get("condition") == Some(&"true".to_string())
        });
        ...
        // Test true condition
        context.metadata.insert("condition".to_string(), "true".to_string());
        let decision = branch_step.execute(&mut context).await.unwrap();
    ...
    async fn test_dowhile_loop_execution() {
        ...
        let condition: ConditionFn = Arc::new(move |context, _| {
            let current_count = context.metadata.get("counter")
        ...
        let loop_step = LoopExecutionStep::new(
            Box::new(increment_step),
            condition,
            LoopType::DoWhile,
        );
        
        let mut context = WorkflowContext::new(10);
        context.metadata.insert("counter".to_string(), "0".to_string());
    ...
    fn test_workflow_builder_fluent_api() {
        let condition: ConditionFn = Arc::new(|context, _| {
            context.metadata.contains_key("should_branch")
        });
